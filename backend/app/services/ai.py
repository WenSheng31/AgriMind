...
from ..schemas.ai import ImageData, ToolUsage
...
- 涉及天氣時，使用 get_weather（支援縣市、鄉鎮、氣象站名，可查即時觀測、鄉鎮預報、農業氣象含積溫）
...
def save_chat_log(user_id: int, session_id: str, role: str, content: str, tool_calls_json: str = None, images_json: str = None):
    """儲存對話紀錄到資料庫，失敗不影響使用者"""
    try:
        db = SessionLocal()
        try:
